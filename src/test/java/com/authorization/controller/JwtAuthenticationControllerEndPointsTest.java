...
class JwtAuthenticationControllerEndPointsTest {
	...
	void testAuthorizedGenerateToken() throws Exception {
		...
		User user = new User(1, "sindhu", "sindhu");
		...
		when(userDetailsService.loadUserByUsername("sindhu")).thenReturn(details);
		ObjectMapper mapper = new ObjectMapper();
		String jsonString = mapper.writeValueAsString(new JwtRequest("sindhu", "sindhu"));
	...
	void textExistingUserAuthenticate() throws Exception {
		User user = new User(1, "sindhu", "pass");
		UserDetails details = new org.springframework.security.core.userdetails.User(user.getUserName(),
				user.getPassword(), new ArrayList<>());
		UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(
				"sindhu", "sindhu");
		when(authenticationManager.authenticate(new UsernamePasswordAuthenticationToken("sindhu", "sindhu")))
				.thenReturn(usernamePasswordAuthenticationToken);
		when(userDetailsService.loadUserByUsername("sindhu")).thenReturn(details);
		when(jwtTokenUtil.getUsernameFromToken("token")).thenReturn("sindhu");
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/api/authenticate").contentType(MediaType.APPLICATION_JSON)
				.content(mapper.writeValueAsString(new JwtRequest("sindhu", "pass")))).andExpect(status().isUnauthorized());
